...
export default async function SponsorsPage({
  ...
  const currentSponsors = [
    { name: "Apex Sports Gear", logoText: "APEX", tier: "gold" as const, description: isIt ? "Fornitura di abbigliamento e attrezzature sportive all'avanguardia per atleti di tutto il mondo." : "Providing state-of-the-art sports apparel and gear to athletes worldwide.", url: "https://example.com" },
    { name: "Velocity Energy", logoText: "VELOCITY", tier: "gold" as const, description: isIt ? "Alimenta i campioni con bevande sportive salutari ed integratori energetici." : "Fueling champions with healthy sports drinks and energy boosters.", url: "https://example.com" },
    { name: "Nova Watches", logoText: "NOVA", tier: "silver" as const, description: isIt ? "Cronometrista ufficiale di eventi motoristici globali e tornei di campionato." : "Official timekeeper of global racing events and championship tournaments.", url: "https://example.com" },
    { name: "Omni Health", logoText: "OMNI", tier: "silver" as const, description: isIt ? "Soluzioni innovative di fisioterapia e programmi di terapia per il recupero sportivo." : "Innovative physiotherapy solutions and sports recovery therapy programs.", url: "https://example.com" },
    { name: "Summit Drinks", logoText: "SUMMIT", tier: "bronze" as const, description: isIt ? "Acqua minerale pura di sorgente alpina, che mantiene gli atleti idratati." : "Pure mineral spring water from alpine ridges, keeping athletes hydrated.", url: "https://example.com" }
  ]

  const packages = [
    {
      name: isIt ? "Posizionamento Bronzo" : "Bronze Placement",
      price: "$299",
      impressions: "10,000+",
      features: isIt ? [
        "Banner laterale standard (300x250)",
        "Rotazione su tutti i resoconti sportivi",
        "Dashboard dei report analitici",
    ...
    {
      name: isIt ? "Caratteristica Argento" : "Silver Feature",
      price: "$699",
      impressions: "50,000+",
      features: isIt ? [
        "Banner pubblicitari in evidenza in home page",
        "Posizionamento annunci a metà articolo",
        "Copertura di tag di contenuti co-brandizzati",
        "Consulente analitico dedicato",
        "Priorità di caricamento degli annunci"
      ] : [
    ...
    {
      name: isIt ? "Oro Premium" : "Gold Premium",
      price: "$1,499",
      impressions: "200,000+",
      features: isIt ? [
        "Banner premium fisso in home page",
        "Spazio in evidenza nella newsletter settimanale",
        "Articolo di recensione editoriale personalizzato",
        "Feed di report API diretti",
        "Team di supporto clienti dedicato"
      ] : [
      ...
      <section className="bg-brand-dark text-white rounded-2xl p-8 md:p-12 text-center relative overflow-hidden shadow-md">
        ...
        <div className="max-w-2xl mx-auto flex flex-col items-center gap-4 relative z-10">
          <span className="inline-flex items-center gap-1 bg-brand-red/10 border border-brand-red/20 text-brand-red px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">
            ...
            {isIt ? "Collabora con SportsPulse" : "Partner With SportsPulse"}
          </span>
          <h1 className="font-headline text-3xl md:text-5xl lg:text-6xl font-extrabold uppercase tracking-tight text-white leading-none">
            {isIt ? "Aumenta la Visibilità del Tuo Brand" : "Boost Your Brand Reach"}
          </h1>
          <p className="text-sm md:text-base text-neutral-300 leading-relaxed font-light">
            {isIt 
              ? "Fai pubblicità con la piattaforma di notizie sportive in più rapida crescita. Raggiungi tifosi, analisti di statistiche e fantallenatori altamente coinvolti in tutto il mondo."
            ...
            >
              ...
              <div>
                ...
                <div className="flex items-baseline gap-1 my-4">
                  ...
                  <span className="text-xs text-neutral-500 font-semibold">{isIt ? "/ mese" : "/ month"}</span>
                </div>
                ...
                <div className="bg-neutral-50 border border-neutral-100 rounded-lg px-3 py-2 text-xs font-semibold text-neutral-600 mb-6 flex items-center justify-between">
                  <span>{isIt ? "Impression garantite:" : "Guaranteed impressions:"}</span>
              ...
              <Link href={`/${lang}/contact`} className="w-full">
                <Button variant={pkg.featured ? "default" : "outline"} className="w-full font-bold">
                  {isIt ? "Richiedi Informazioni" : "Inquire Now"}
                </Button>
              </Link>
            </div>
          ))}
      ...
      <section className="flex flex-col gap-6">
        <div className="border-b border-neutral-200 pb-3">
          <h2 className="font-headline text-2xl md:text-3xl font-extrabold uppercase text-brand-dark">
            {isIt ? "I Nostri Partner Ufficiali" : "Our Official Partners"}
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {currentSponsors.map((sponsor) => (
            <SponsorCard
              ...
              buttonText={t.sponsors.visitWebsite}
            />
          ))}
        </div>
      </section>
